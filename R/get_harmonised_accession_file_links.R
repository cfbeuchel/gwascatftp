...
get_harmonised_accession_file_links <- function(
  ...
  stopifnot(
    "Have to supply accession and list!" = (!is.na(study_accession) |
      all(!is.na(directory_list)))
  )
  if (
    isFALSE(
  ...
  if (isFALSE(list_all_files)) {
    relative_path <- grep(
      pattern = glue::glue("/{study_accession}/"),
      x = harmonised_list,
      fixed = TRUE,
      value = TRUE
    )
  ...
  harmonised_data_directory <- glue::glue(
    "{accession_bucket}/{study_accession}/harmonised"
  )
  ...
  harmonised_files_full_links <- glue::glue(
    "{ftp_root}/{accession_bucket}/{study_accession}/harmonised/{harmonised_files_list}"
  )
